...
#include "ocean.hpp"
#include "texture_mgr.hpp"          // ← для картинки
...
void Predator::update(Ocean& oc,int x,int y)
{
    ...
    /* старение */
    if(age_ >= AGE_LIMIT && oc.population().pred >= MAX_POP){   // ← field name fixed
        oc.clearCell(x,y);
        return;
    }
    ...
    if(!veryHungry && isHerb(oc,nx,ny)) return;   // сытый не атакует
...
{
    static const sf::Texture& tex = TextureMgr::inst().get("predator.png"); // ← added
    sf::Sprite spr(tex);
