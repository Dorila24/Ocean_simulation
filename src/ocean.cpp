...
Ocean::Ocean(int w,int h): pImpl(std::make_unique<Impl>(w,h))
{
    ...
    /* мир без начального ковра — столбы вырастут сами */
...
static void tryGrowAlgae(Ocean& oc,int x)
{
    ...
    const int ySand   = oc.height() - SAND_ROWS;        // первая клетка ПОД песком
    const int yBottom = ySand - 1;                      // самая нижняя доступная
    const int yTopLim = oc.minAlgaeRow();               // выше расти нельзя

    /* шаг 1: находим вершину существующего столба (или песок) */
    int y = yBottom;
    while(y >= yTopLim && dynamic_cast<Algae*>(oc.entityAt(x,y)))
        --y;                                            // поднимаемся до первой НЕ Algae
    ++y;                                                // y → верхушка (или yBottom + 1)

    /* y сейчас указывает на верхнюю Algae или yBottom + 1 если столба не было.
       Следующая растущая клетка — на 1 выше. */
    int yGrow = y - 1;
    if(yGrow < yTopLim)      return;                    // достигли лимита высоты
    if(!oc.isEmpty(x,yGrow)) return;                    // занято (рыба и т.д.)

    oc.spawn<Algae>(x,yGrow);                           // посадили новую клетку
}
